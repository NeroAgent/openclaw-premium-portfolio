#!/usr/bin/env python3
"""
openfang hands disable — Disable a Hand.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Disable a Hand")
    ...
    try:
        result = subprocess.run([openfang, "hands", "disable", args.hand_name], capture_output=True, text=True, timeout=10)
        if result.returncode != 0:
            print(f"[ERROR] {result.stderr}", file=sys.stderr)
            return 1
        print(f"✅ Disabled hand: {args.hand_name}")
        return 0
