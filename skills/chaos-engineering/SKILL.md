...
Adversarial training & auto-healing system for OpenClaw agents. Induce controlled failures to test resilience, then watch shadow skills auto-recover. Includes thermal dashboard for system health monitoring.

## Overview

The Chaos Engineering skill implements a Chaos Monkey pattern for OpenClaw:
...
- **Thermal dashboard**: Terminal-based health visualization
...
Each break creates a **shadow skill** (JSON manifest) that knows how to heal itself. Shadow skills are stored in `~/.openclaw/chaos/shadow_skills/` and can be reused.
...
2. Logs event to `~/.openclaw/chaos/battle_log.jsonl`
3. Generates shadow skill with healing command
4. Shadow skill can be triggered manually via `chaos_heal`
...
- OpenClaw memory stack (WAL, buffer, token utilization)
- Skill registry health
- Shadow skill count
- Recent chaos events
- Thermal gradient based on system health

Run with `format="ansi"` for colored terminal output, or `format="text"` for plain.
...
# 3. Check WAL is gone
tool("memory-stack-core", "wal_read", {"limit": 5})  # should be empty

# 4. Heal
tool("chaos-engineering", "chaos_heal", {})

# 5. Verify recovery from buffer?
...
- `~/.openclaw/chaos/chaos_engine/battle_log.jsonl` — event log
- `~/.openclaw/chaos/shadow_skills/*.json` — auto-generated healing skills
